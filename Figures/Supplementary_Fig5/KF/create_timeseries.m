...
for trial = trial_list
    ...
    [t_ori, y_ori] = ode15s(@(t,y) KF(t,y), tspan, initial);
    ...
    for i = 1:length(initials(:,1))
        [t1, y1] = ode15s(@(t,y) KF(t,y), tspan, initials(i,:));
    ...
    % save data
    filename = ['KF_timeseries_Trial',num2str(trial)];
    save(filename, 'y_total', 't', 'time_interval', 'period', 'num_component', 'num_data')
    filename = ['KF_timeseries_fit_0_Trial',num2str(trial)];
    save(filename, 'y_total', 't', 'time_interval', 'period', 'num_component', 'num_data')
end
