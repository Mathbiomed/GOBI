...
for trial = trial_list
    ...
    dimension = 1;
    thres_noise = 1e-5;
    load('Fr_timeseries_Trial1') % for import parameters
    noise_list = [0:2:20];
    ...
    for noise_percent = noise_list
        ...
        %% load data
        filename = ['Fr_timeseries_fit_',num2str(noise_percent),'_Trial',num2str(trial)];
        load(filename)
        ...
        %% Save results
        filename = ['Fr_results_dim1_',num2str(noise_percent),'_Trial',num2str(trial)];
