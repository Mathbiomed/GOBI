...
for j = 1:length(y_total)
    ...
    for i = 1:num_pair
        ...
        % assume positive self-regulation for the component 'f'
        if ed == 1
            [score_list, t_1, t_2] = RDS_ps_dim1(y_target(:,st), y_target(:,ed), t, time_interval);
        else
...
%% Save results
filename = ['RDS_ps_dim1_',num2str(num_data)];
