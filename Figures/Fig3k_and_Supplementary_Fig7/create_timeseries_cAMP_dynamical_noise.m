...
for trial = trial_list
    ...
    for noise_percent = noise_list
        ...
        for i = 1:length(initials(:,1))
            [t1, y1] = ode45(@(t,y) cAMP_noise(t,y,noise_percent/100,mean_range(1),mean_range(2),mean_range(3),mean_range(4),mean_range(5),mean_range(6),mean_range(7)), tspan, initials(i,:));
            for j = 1:num_component
                y1(:,j) = y1(:,j) - min(y1(:,j));
                y1(:,j) = y1(:,j) / max(y1(:,j));
            end
            y_total_noise{end+1} = y1;
        end
        t = t1;
        ...
        % save data
        filename = ['./Data_dynamical/cAMP_timeseries_dynamical_',num2str(noise_percent),'_Trial',num2str(trial)];
        save(filename, 'y_total_noise', 't', 'time_interval','noise_percent','num_component','num_data','period')

        %filename = ['./Data_dynamical/cAMP_timeseries_Trial',num2str(trial)];
        %filename = ['cAMP_timeseries_large'];
