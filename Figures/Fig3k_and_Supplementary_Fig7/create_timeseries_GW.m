...
for trial = trial_list
    ...
    [t_ori, y_ori] = ode15s(@(t,y) Goodwin(t,y), tspan, initial);
    ...
    for i = 1:length(initials(:,1))
        [t1, y1] = ode15s(@(t,y) Goodwin(t,y), tspan, initials(i,:));
    ...
    % save data
    filename = ['./Data_original/GW_timeseries_Trial',num2str(trial)];
    %filename = ['./Data_original/GW_timeseries'];
