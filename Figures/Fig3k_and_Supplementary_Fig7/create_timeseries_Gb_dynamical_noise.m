...
for trial = trial_list
    ...
    for noise_percent = noise_list
        ...
        for i = 1:length(initials(:,1))
            [t1, y1] = ode45(@(t,y) Goldbeter_noise(t,y,noise_percent/100,mean_range(1),mean_range(2),mean_range(3),mean_range(4),mean_range(5)), tspan, initials(i,:));
        ...
        % save data
        filename = ['./Data_dynamical/Gb_timeseries_dynamical_',num2str(noise_percent),'_Trial',num2str(trial)];
