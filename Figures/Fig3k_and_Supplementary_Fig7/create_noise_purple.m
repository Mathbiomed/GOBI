...
for trial = trial_list
    ...
    for noise_percent = noise_list
        ...
        for i = 1:length(y_total)
            ...
            cn = dsp.ColoredNoise('purple',length(t),num_component,OutputDataType='single');
            noise = cn();
            noise = (noise_percent / 100) * noise;
            y_tmp_noise = y_tmp.*(noise+1);
            y_total_noise{end+1} = y_tmp_noise;
        end
        filename = ['./Data_purple/',system,'_timeseries_purple_',num2str(noise_percent),'_Trial',num2str(trial)];
