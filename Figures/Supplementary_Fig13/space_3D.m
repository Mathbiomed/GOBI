...
%% load data
load('3D_timeseries.mat')

%% parameter
thres_noise = 1e-7;
size_window = length(t);
I_total_XY = zeros(num_data, size_window, size_window, 8);
...
for i = 1:num_data
    ...
    [score_list, t_1, t_2] = RDS_dim3(y_target(:,2), y_target(:,3), y_target(:,4), y_target(:,1), t_target, time_interval);
    for k = 1:8
    ...
    for m = 1:size_window
        for n = 1:size_window
            if abs(score_XY(m,n)) > thres_noise
                %trans_XY = [trans_XY ; [X_var(m), X_var(n),Y_var(m), Y_var(n), score_XY(m,n)]];
...
filename = ['figure_3D_',num2str(num_data_used),'.png'];
saveas(gcf,filename)