...
dt_pre = linspace(0,period, period/time_interval_pre+1);
dt = linspace(0,period, period/time_interval+1);
num_data = 100;
...
for i = 1:num_data
    ...
    d_pre = rand([length(dt_pre),1])*2-1;
    d_tmp = spline(dt_pre,d_pre,dt);
    y_0 = rand(1)*2-1;
    
    % create other timeseries
    y_tmp = zeros(length(tspan),4);
    [t0, y0] = ode45(@(t,y) example_3D(t,y,bt,b_tmp,ct,c_tmp,dt,d_tmp), tspan, y_0);
    y_tmp(:,1) = y0;
    y_tmp(:,2) = b_tmp;
    y_tmp(:,3) = c_tmp;
    y_tmp(:,4) = d_tmp;
...
hold on
plot(t0, y2(:,4), 'k','linewidth',2)

%% save data

t = tspan.';
filename = '3D_timeseries';
