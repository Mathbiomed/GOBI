...
for i = 1:num_data
    ...
    [score_list, t_1, t_2] = RDS_dim1(y_target(:,2), y_target(:,1), t_target, time_interval);
    for k = 1:2
...
%% linearlize (t,t*) into X(t),X(t*)
disp('transformation')
trans_total_XY = cell(num_data,1);

for i = 1:num_data
    score_XY_pos = reshape(I_total_XY(i,:,:,1), [size_window, size_window]);
    score_XY_neg = reshape(I_total_XY(i,:,:,2), [size_window, size_window]);
    ...
    for m = 1:size_window
        for n = 1:size_window
            if abs(score_XY_pos(m,n)) > thres_noise
                trans_XY = [trans_XY ; [t(m), t(n), score_XY_pos(m,n)]];
            end
        end
    end        
...
filename = ['figure_1D_',num2str(num_data_used),'.png'];
saveas(gcf,filename)