...
for trial = trial_list
    ...
    [t_ori, y_ori] = ode45(@(t,y) repressilator(t,y), tspan, initial);
    ...
    hold on
    plot(t_ori, y_ori(:,5), 'k')
    hold on
    plot(t_ori, y_ori(:,6), 'r')
    hold on
    ...
    for i = 1:num_component
        ...
        %initials(i,:) = initials(i,:) / sum(initials(i,:));
    ...
    for i = 1:length(initials(:,1))
        [t1, y1] = ode45(@(t,y) repressilator(t,y), tspan, initials(i,:));
    ...
    % save data
    filename = ['rep_timeseries_Trial',num2str(trial)];
