...
addpath('../../../GOBI')
addpath('../../../Timeseries_Data/in_silico/cAMP')
...
%% load data
filename = 'cAMP_timeseries_Trial0';
load(filename)

%% create pair
component = [1:num_component];
component_list_dim1_tmp = nchoosek(component, dimension);
component_list_dim1 = [];
for i = 1:length(component_list_dim1_tmp(:,1))
    for j = 1:length(component)
        if ~ismember(component(j), component_list_dim1_tmp(i,:))
            component_list_dim1 = [component_list_dim1 ; [component_list_dim1_tmp(i,:),component(j)]];
        end
    end
end
