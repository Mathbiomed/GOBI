...
num_pair = length(component_list_dim3);

%% for every data, calculate regulation detection score
disp('calculate regulation detection score...')
S_total_list = zeros(num_pair,8,length_data);
L_total_list = zeros(num_pair,8,length_data);
for i = 1:length_data
    y_target = cell2mat(y_total(i));
    t_target = t(1:length(y_target(:,1)));
    
    S_total = zeros(num_pair,8);
    L_total = zeros(num_pair,8);
    for j = 1:length(component_list_dim3(:,1))
        st1 = component_list_dim3(j,1);
        st2 = component_list_dim3(j,2);
        st3 = component_list_dim3(j,3);
        ed = component_list_dim3(j,4);
