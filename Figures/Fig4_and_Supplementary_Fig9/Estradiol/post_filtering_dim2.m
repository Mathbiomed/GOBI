...
delta_total = zeros(num_data, 2, num_candidate_delta);
...
%for i = 2
    ...
    delta_total(:,1,i) = delta_1;
    delta_total(:,2,i) = delta_2;
    ...
    if ~isempty(rmmissing(delta_1))
        delta_1_tmp = delta_1;
        delta_1_tmp(find(isnan(delta_1_tmp) == 1)) = 1;
        %[p1,h1,stats1] = signrank(rmmissing(delta_1_tmp),-1e-3,'tail','right');
        p1 = length(find(rmmissing(delta_1) < -1e-3)) / length(rmmissing(delta_1));
        %[p1,h1,stats1] = signrank(rmmissing(delta_1),-1e-3,'tail','right','alpha',0.01,'method','approximate');
        
    end
    if ~isempty(rmmissing(delta_2))
        delta_2_tmp = delta_2;
        delta_2_tmp(find(isnan(delta_2_tmp) == 1)) = 1;
        
        %[p2,h2,stats2] = signrank(rmmissing(delta_2_tmp),-1e-3,'tail','right');
...
num_boot = 100;
surrogate_list = [];
boot_total = zeros(num_data,2,num_candidate_boot);
